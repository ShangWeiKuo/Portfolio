rm(list = ls(all = T)); gc()
pacman::p_load(dplyr, caret, stringr)
...
# Searching the columns containing "some" white spaces, 
...
# 以 70:30 的比例切分資料為「訓練(training)」與「測試(testing)」資料集
# Random seed 設定為 2021
set.seed(2021)
...
## 儲存Rdata檔
save(loan, train_loan, test_loan, file = "data_preprocess.rdata")
save.image(file = "data_preprocess_backup.rdata")
